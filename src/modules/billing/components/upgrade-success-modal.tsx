...
import { Sparkles, Zap, Globe, BarChart3, X } from 'lucide-react'
...
const PRO_FEATURES = [
  { icon: Globe, label: 'Unlimited published pages' },
  { icon: Zap, label: 'AI-powered block generation' },
  { icon: BarChart3, label: 'Analytics & visitor insights' },
] as const
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={dismiss}
          className="absolute right-4 top-4 rounded-lg p-1 text-white/30 transition-colors hover:text-white/60"
          aria-label="Close"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <button
          onClick={dismiss}
          className="w-full rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition-opacity hover:opacity-90"
        >
          Start building
        </button>
      </div>

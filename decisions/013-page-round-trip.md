...
**Rejected:**
- *`/dashboard/[pageId]`* — natural breadcrumb but the route moves in Phase 2. Creates a throwaway route.
- *Both routes* — over-engineering for proving a data path.
...
**Rejected:**
- *Trust the DB, cast the type* — works until it doesn't. When it fails, debugging is painful because the error is far from the cause. Violates the "validate at boundaries" principle.
...
**Rejected:**
- *Rendered preview* — same throwaway lifespan as JSON, but more code. Rendering logic would be rewritten for dnd-kit in Phase 2.
- *JSON + preview toggle* — most code, toggle UI is throwaway.
